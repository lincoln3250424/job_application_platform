...
export function ResumeManager({ resumes }: { resumes: ResumeRow[] }) {
  ...
  async function handleAdd(e: FormEvent) {
    ...
    try {
      const res = await fetch("/api/resumes", {
        ...
        body: JSON.stringify({ label: label.trim() || "My resume", content: content.trim() }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => null);
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h2 className="font-display font-bold text-xl mb-4">Saved resumes</h2>
        {resumes.length === 0 ? (
          <p className="text-ink-soft text-sm italic">
            No saved resumes yet — add one above or paste a new one when you
            open an application.
          </p>
        ) : (
